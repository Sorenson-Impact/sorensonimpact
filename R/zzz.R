...
check_si_up_to_date <- function() {
  ...
  if(x$installed != x$available) {
    cli::cli_alert_info(cli::bg_red(cli::col_white(cli::style_bold("A newer version of \`sorensonimpact\` is available.  Run \`update_packages(\"sorensonimpact\")\` to update."))))
  }
